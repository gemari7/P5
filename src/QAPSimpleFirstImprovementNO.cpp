...
bool MQKPSimpleFirstImprovementNO::findOperation(MQKPInstance &instance, MQKPSolution &solution, MQKPChangeOperation &operation) {
	...
	for(int i=0;i<numObjs;i++){ //Exploramos todos los objetos
		for(int j=1;j<=numKnapsacks;j++){ //Exploramos todas las mochilas
			if(evaluator.computeDeltaFitness(instance,solution,perm[i],j)>0){ //El deltaFitness es positivo
				oaOperation->setValues(perm[i],j,evaluator.computeDeltaFitness(instance,solution,perm[i],j)); //Actualizamos los valores de la operacion
